<template>
  <div class="settlement-amount-container">
    ...
    <div class="amount">
      <div v-if="adult" class="amount-item">
        <div v-if="feeType === TYPE_FEE.AGE" class="title">
          {{ $t("one_adult", { number: adult }) }}
        </div>
        <div v-else class="title">
          {{ $t("one_male", { number: adult }) }}
        </div>
        <div class="price">{{ priceDisplay(adultPrice) }}{{ $t("yen") }}</div>
      </div>
      <div v-if="children" class="amount-item">
        <div v-if="feeType === TYPE_FEE.AGE" class="title">
          {{ $t("one_child", { number: children }) }}
        </div>
        <div v-else class="title">
          {{ $t("one_female", { number: children }) }}
        </div>
