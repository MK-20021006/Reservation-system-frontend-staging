    ...
    <client-only>
      ...
      >
        <el-scrollbar max-height="400px">
          <div v-loading="loading" class="show-time">
            <div class="header">
            ...
            <div v-for="item in data" :key="item" class="content">
              ...
              >
                <ButtonRed
                  ...
                  @click="handleBooking(item.item.date, item.item?.id)"
                />
              </div>
              <div
                v-if="
...
import { EventTimeInterface } from "~/../../src/front/models";
...
const { dialogVisible, dateMonth, width, dataTime, eventId, data, loading } =
  toRefs(props);
...
const handleBooking = (date: string, time_event_id: number) => {
  emits("handleBooking", date, time_event_id);
};
</script>
