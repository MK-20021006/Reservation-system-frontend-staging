<template>
  <div>
      ...
      <template #default>
        ...
        <el-carousel-item v-for="item in dataImage" :key="item">
          <div class="show-icon-next">
            <img v-if="item.file" :src="item.file.url" />
    ...
    <div class="smail-image">
      ...
      >
        <img v-if="itemImage.file" :src="itemImage.file.url" />
        <img v-else :src="itemImage.url" />
      </div>
      <div
...
const dataImage = computed(() => {
  ...
  imageLength.value = 10 - dataGet.length;
  return dataGet;
});
