<template>
  <div v-loading="loadingChange" class="connect-tabelog">
    <connect-template-crawl
      ...
      :is-hidden-button-connect="
        !(formData.setting_mail_forward && formData.crawled_seat)
      "
      ...
      <template #connect-type>
        ...
        >
          ...
          <template v-if="formData.status === STATUS_CONNECT.SUCCESSFUL">
            <div-inline
              :label="'title_screen.seat_list'"
              :class-name="'option-connect'"
            >
              <template #form-item>
                <a class="text-link" :href="formData.url_seat">
                  {{ formData.url_seat }}
                </a>
              </template>
            </div-inline>
            <div-inline
              :label="'seats_on_external'"
              :class-name="'option-connect'"
            >
              <template #form-item>
                <el-radio-group
                  v-model="formData.crawled_seat"
                  @change="handleChangeSeatStatus"
                >
            ...
            <div class="mail-forward">
              ...
              <div class="text-note-check-mail">
                ...
                <span class="text-highlight">{{
                  $t("text_note_check_mail_tabelog")
                }}</span>
                <span>{{ $t("text_note_check_mail_2") }}</span>
              </div>
              <div-inline
                :label="'store.tabelog_linkage'"
                :class-name="'option-connect'"
              >
                <template #form-item>
                  <a class="text-link link" :href="URL_TABELOG" target="_blank">
                    {{ URL_TABELOG }}
                  </a>
                </template>
              </div-inline>
...
import { URL_TABELOG } from "~/../../src/shared/constants";
...
const formData = reactive({
  ...
  url_seat: urlSeat,
  crawled_seat: data.value?.crawled_seat || 0,
...
const handleChangeMailStatus = async () => {
  try {
    ...
    await storeSettingRequest.settingMailForwardTabelog({
...
const handleChangeSeatStatus = async () => {
  try {
    loadingChange.value = true;
    await storeSettingRequest.settingCrawlSeatTabelog({
      crawled_seat: !!formData.crawled_seat,
    });
  } catch (e) {
    if (formData.setting_mail_forward) {
      formData.crawled_seat = 1;
    } else {
      formData.crawled_seat = 0;
    }
  } finally {
    loadingChange.value = false;
  }
...
const checkConnection = async (form: FormInstance) => {
  ...
  await form.validate(async (valid) => {
    if (valid) {
      try {
        loading.value = true;
        await bookingRequest.synchronizedDataTabelog();
...
watch(data, (value) => {
  if (value) {
    ...
    formData.crawled_seat = value.crawled_seat || 0;
