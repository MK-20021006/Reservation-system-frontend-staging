            ...
            <div class="mail-forward">
              ...
              <div class="text-note-check-mail">
                ...
                <span class="text-highlight">{{
                  $t("text_note_check_mail_gurunabi")
                }}</span>
                <span>{{ $t("text_note_check_mail_2") }}</span>
              </div>
              <div-inline
                :label="'store.gurunavi'"
                :class-name="'option-connect'"
              >
                <template #form-item>
                  <a
...
import { URL_GURUNABI } from "~/../../src/shared/constants";
...
const handleChangeMailStatus = async () => {
  try {
    ...
    await storeSettingRequest.settingMailForwardGurunavi({
...
const handleChangeSeatStatus = async () => {
  try {
    loadingChange.value = true;
    await storeSettingRequest.settingCrawlSeatGurunavi({
...
const checkConnection = async (form: FormInstance) => {
  ...
  await form.validate(async (valid) => {
    if (valid) {
      try {
        loading.value = true;
        await bookingRequest.synchronizedDataGurunavi();
