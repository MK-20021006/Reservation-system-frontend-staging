      ...
      <el-scrollbar>
        <div
          ...
          :class="[
            ...
            { 'max-booking': checkMaxBooking(day.time) },
...
const checkMaxBooking = (time: string) => {
  const timeFormat = moment(time).format("YYYY-MM-DD");
  return reservationStore.reservationSalon.find((item) => {
    return (
      moment(timeFormat, "YYYY-MM-DD").isSame(
        moment(item.start_time).format("YYYY-MM-DD")
      ) && item.reservation?.elder_quantity_device
    );
  });
};
